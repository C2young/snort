...
/* $Id: sp_ip_same_check.c,v 1.6 2002/05/13 20:14:06 chrisgreen Exp $ */
...
void IpSameCheckInit(char *data, OptTreeNode *otn, int protocol)
{
    ...
    otn->ds_list[PLUGIN_IP_SAME_CHECK] = (IpSameData *) calloc(sizeof(IpSameData), sizeof(char));
...
void ParseIpSame(char *data, OptTreeNode *otn)
{
    ...
    if (*data) {
	    FatalError("ERROR %s (%d): arg '%s' not required\n", file_name, file_line, data);
    }
}

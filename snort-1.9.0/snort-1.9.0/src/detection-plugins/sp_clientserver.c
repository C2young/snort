/* $Id: sp_clientserver.c,v 1.13 2002/08/10 03:49:05 roesch Exp $ */
...
void FlowInit(char *data, OptTreeNode *otn, int protocol)
{
    ...
    {
        FatalError("ERROR %s(%d): Cannot check flow connection "
                "for non-TCP traffic\n", file_name, file_line);
    }
...
void ParseFlowArgs(char *data, OptTreeNode *otn)
{
    ...
    if(str == NULL)
    {
        FatalError("ERROR: ParseFlowArgs: Can't strdup data\n");
    }
...
void InitFlowData(OptTreeNode * otn)
{
    ...
    {
        FatalError("ERROR: FlowData calloc Failed!\n");
    }
}
...
int CheckFromClient(Packet *p, struct _OptTreeNode *otn, OptFpList *fp_list)
{
    ...
    DebugMessage(DEBUG_STREAM, "Packetflags: 0x%X\n", p->packet_flags);
    ...
    DEBUG_WRAP(DebugMessage(DEBUG_STREAM, "CheckFromClient: returning 0\n");               
               DebugMessage(DEBUG_STREAM, "from server?: 0x%X\n",
                            p->packet_flags & PKT_FROM_SERVER);
               );    

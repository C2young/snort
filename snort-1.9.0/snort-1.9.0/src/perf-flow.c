...
/*
** $Id: perf-flow.c,v 1.2 2002/08/18 16:16:31 chrisgreen Exp $
**
** perf-flow.c
**
**
...
{
    sfFlow->pktLenCnt = (sfperf_uint64_t*)malloc(sizeof(sfperf_uint64_t) * (SF_MAX_PKT_LEN+1) );
    if( sfFlow->pktLenCnt )
        memset(sfFlow->pktLenCnt,0,sizeof(sfperf_uint64_t)*(SF_MAX_PKT_LEN+1));
    
    sfFlow->pktTotal = 0;
    sfFlow->byteTotal = 0;

    sfFlow->portTcpSrc = (sfperf_uint64_t*)malloc(sizeof(sfperf_uint64_t) * SF_MAX_PORT);
    if( sfFlow->portTcpSrc )
        memset( sfFlow->portTcpSrc, 0, sizeof(sfperf_uint64_t)*SF_MAX_PORT );

    sfFlow->portTcpDst = (sfperf_uint64_t*)malloc(sizeof(sfperf_uint64_t) * SF_MAX_PORT );
    if( sfFlow->portTcpDst )
        memset( sfFlow->portTcpDst, 0, sizeof(sfperf_uint64_t)*SF_MAX_PORT );
    
    sfFlow->portTcpHigh=0;
    sfFlow->portTcpTotal=0;


    sfFlow->portUdpSrc = (sfperf_uint64_t*)malloc(sizeof(sfperf_uint64_t) * SF_MAX_PORT );
    if( sfFlow->portUdpSrc )
        memset( sfFlow->portUdpSrc, 0, sizeof(sfperf_uint64_t)*SF_MAX_PORT );

    sfFlow->portUdpDst = (sfperf_uint64_t*)malloc(sizeof(sfperf_uint64_t) * SF_MAX_PORT );
    if( sfFlow->portUdpDst )
        memset( sfFlow->portUdpDst, 0, sizeof(sfperf_uint64_t)*SF_MAX_PORT );
    
    sfFlow->portUdpHigh=0;
    sfFlow->portUdpTotal=0;

    sfFlow->typeIcmp = (sfperf_uint64_t *)calloc(256, sizeof(sfperf_uint64_t));
    if(!sfFlow->typeIcmp)
    {
       return 1;
    }

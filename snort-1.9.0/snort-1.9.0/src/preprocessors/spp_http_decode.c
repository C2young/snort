...
**
** $Id: spp_http_decode.c,v 1.48 2002/09/25 19:56:54 chrisgreen Exp $
*/
...
static u_int lookup_whitespace[256];
static int   lookup_hexvalue[256];
...
void SetupHttpDecode()
{
    ...
    DEBUG_WRAP(DebugMessage(DEBUG_HTTP_DECODE, "Preprocessor: HttpDecode in setup...\n"););
}
...
{
    int idx;
    DEBUG_WRAP(DebugMessage(DEBUG_HTTP_DECODE, "Preprocessor: HttpDecode Initialized\n"););
    ...
    /* the hexvalues table doubles as a isxdigit() replacement 
       as well, allowing us to kill two birds with one stone and
       cut out a lot of redundant processing time                */
     ...
     * We first reset the entire port list and then make port 80 the default.
     */
...
Purpose: Inspect the packet to see if we should process the packet
or not
...
static inline int CheckDecodeHTTP(Packet *p) {
    ...
    {
        DebugMessage(DEBUG_HTTP_DECODE, "from client!\n");
    }
#endif /* DEBUG */
...
static void PreprocUrlDecode(Packet *p)
{
    ...
    /* register */ int hex1, hex2;  /* hex lookup values */
    char logMessage[180];
    u_int overlong_flag;
    Event event;
    ...
    index = (char *) p->data; /* index into the data portion of the packet */
	...
	/* the request had an abnormally-large HTTP method and/or
	   amount of separating whitespace.  This is only typical
	   of attacks:
	   - attempting a buffer overflow
	   - attempting IDS evasion
	   - attempting to screw with the HTTP server logs
	*/
        if(internal_alerts)
        {
            snprintf(logMessage, sizeof(logMessage) - 1,
                     HTTP_DECODE_LARGE_METHOD_STR);
    ...
    if(index == end)
    {
	/* gross...malformed request with no URI? 
	   I guess a very weird HTTP 0.9 request could
	   trigger this, but odds are something evil
	   is happening. 
	*/
        if(internal_alerts)
        {
            snprintf(logMessage, sizeof(logMessage) - 1,
                     HTTP_DECODE_MISSING_URI_STR);
            SetEvent(&event, GENERATOR_SPP_HTTP_DECODE, 
    ...
    /* this is a speed hack to avoid unnecessary *cur=*index
       copying found in the next loop.  Think of it as a
       precursor strchr, without having to readjust pointers
       and run strlen's

       Would be included if we could munge packets in place but we
       need the original packet data to stay around

       do {
       if(*index == '%' || lookup_whitespace[(u_int)(*index)])
       break;
       if(*index == '?'  && end_on_url_param)
       break;
       if(*index == '\\' && iis_flip_slash)
       (*index) = '/';
       } while (index++ < end);
    ...
    */
	...
	if(*index == '%')
	{
	    if( ((*(index+1)) == 'u' || (*(index+1)) == 'U')
		&& check_alt_unicode)
	    {
		/* alternate IIS unicode encoding.
		   We convert '%uC0AF' to '%C0%AF',
		   and then decode it normally      
		   NOTE: this opens us up to decoding
		   discrepencies if any hex is invalid    */
		...
		*(cur+3) = '%';				
	    ...
	    /* Check Me */
	    if(check_double_encode && (*(cur+1)=='2')
	       && (*(cur+2)=='5'))
	    {
		...
		*cur = ((hex1<<4)+hex2) & 0xff;
	    ...
	    {
		/* illegal character...they tried to
		   submit a non-valid hex character,
		   with is typical of an anti-IDS
		   subversion or DoS attack         */
                if(internal_alerts)
                {
                    snprintf(logMessage, sizeof(logMessage) - 1,
                             HTTP_DECODE_ILLEGAL_HEX_STR);
                    SetEvent(&event, GENERATOR_SPP_HTTP_DECODE, 
		...
		/* Apache will not continue processing the request,
	...
	else
	{
	    if((*index == '?') && end_on_url_param)
	    {
		/* time to just start copying bytes
		   into the URI buffer */
		*cur = *index;
		while(index < end &&
 ...
 end_decode_loop:
    ...
    *((char *) UriBufs[0].http_version + 10) = '\0';

    if(strncmp(UriBufs[0].http_version, "HTTP/1",6))
    {
	if(*((char *) UriBufs[0].http_version + 8 ) == '1')
	{
	    printf("HTTP/1.1!!!\n");
	}
	else if(*((char *) UriBufs[0].http_version + 8 ) == '0')
	{
	    printf("HTTP/1.0!!! %c\n", *((char *) UriBufs[0].http_version + 8 ));
	} else {
	    printf("special case?!\n");
	}
    ...
    printf("%s:%d ->", inet_ntoa(p->iph->ip_src), p->sp);
    printf("%s:%d ", inet_ntoa(p->iph->ip_dst),  p->dp);
    
    printf("http version: %s\n", (char *) UriBufs[0].http_version);
    return 0; 
#endif /* BUGGER */
	...
	overlong_flag = 0;
        ...
        unidecode((char *) UriBufs[0].uri, (cur - (char *) UriBufs[0].uri),
                  &overlong_flag);

        UriBufs[0].length  = (cur - (char*)UriBufs[0].uri);

        /* 	overlong characters are technically illegal in the UTF-8
		spec, and receiving any means that odds are the person
		is up to no good.  Keep in mind, this is just overlong
...
void CreateNodeList(u_char * servers)
{
    ...
    if(servers == NULL)
    {
        FatalError("ERROR: %s (%d)=> No arguments to "
...
This function attempts to decode a string containing unicode values;
since snort internally uses single-character strings, we have issues
when we attempt to store a multi-byte character; so, instead, we
substitute legitimate multi-byte characters with the character
defined as UNICODE_REPLACEMENT.  This is *much* better than dropping
the character(s) altogether, since that could potentially result
in nastiness (DoS and false-positives, and potentially another
anti-IDS tactic).

*/
static u_int unidecode(char* in, u_int len, u_int* overlong_flag)
{
    unsigned char lead, not_overlong, not_unicode, tmp;
    unsigned int ptr, count, idx, cur;

    ptr = cur = 0;
    not_overlong = 0;	/* is the unicode byte sequence overlong? */
    not_unicode = 0;	/* invalid unicode byte sequence (byte values outside range) */
    *overlong_flag = 0;	/* number of found overlong sequences */

    while(ptr < len)
    {

	if( (unsigned char)in[ptr] >= 0xc0 &&
            (unsigned char)in[ptr] <= 0xfd )
	{
	    lead = in[ptr];
		...
		if ( count >= 5 ) /* assumption: values <=32 bits */
            ...
            else
            {
                ...
                {
                    (*overlong_flag)++;
                    tmp = in[ptr + count - 1] << 6; /* high two bits in previous byte */

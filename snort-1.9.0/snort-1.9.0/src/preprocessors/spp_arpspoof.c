...
/* $Id: spp_arpspoof.c,v 1.17 2002/08/08 23:21:18 chrisgreen Exp $ */
...
void ARPwatchArg(u_char *args)
{
    ...
    if (ipmel == NULL)
    {
        ErrorMessage("ERROR %s %s(%d) => Cannot initialize"
    ...
    if (num_toks != 2)
    {
        ErrorMessage("ERROR %s %s(%d) => Invalid arguments to "
                "arpspoof_detect_host ... ignoring\n", MODNAME, file_name, 
                file_line);
    ...
    {
        ErrorMessage("ERROR %s %s(%d) => Non IP as first argument of "
    ...
    if (num_macbytes < 6)
    {
        ErrorMessage("ERROR %s %s(%d) => Invalid MAC address as second "

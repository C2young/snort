...
typedef struct _Portscan2Data
{
    ...
    HostNode *hostList; /* for ignore-hosts */
...
void InitIgnoreHosts(u_char *hosts)
{
    ...
    toks = mSplit(hosts, " ", 31, &num_toks, '\\');
        
    for(num_hosts = 0; num_hosts < num_toks; num_hosts++)
    {
        if(currentHost != NULL)
        {
            currentHost->nextNode = (struct _hostNode *) calloc(sizeof(HostNode), sizeof(char));
            currentHost = (HostNode *) currentHost->nextNode;
        }
        else
        {
            currentHost = (HostNode *) calloc(sizeof(HostNode), sizeof(char));
            ps2data.hostList = currentHost;
        }
...
IpAddrSet* IgnoreAllocAddrNode(HostNode * host)
{
    ...
    {
        host->address = (IpAddrSet *) calloc(sizeof(IpAddrSet), sizeof(char));
...
void ScanParseIp(char *addr, HostNode *host)
{
    ...
    if(addr == NULL)
    {
        ErrorMessage("ERROR %s(%d) => Undefine address in "
                     "portscan-ignorehosts directive, ignoding.\n", file_name, 
                     file_line);
...
int IsIgnored(Packet *p)
{
...
#endif
    HostNode *currentHost = ps2data.hostList;
    ...
    while(currentHost)
    {
         ...
         * found.
         */
        if(CheckAddrPort(currentHost->address, 0, 0, p,
                         (ANY_SRC_PORT | currentHost->ignoreFlags), CHECK_SRC))
        {
#ifdef DEBUG
            ...
            strncpy(ruleIP, inet_ntoa(
                                      *(struct in_addr*)&(currentHost->address->ip_addr)), 
                    14);
            strncpy(ruleNetMask, 
                    inet_ntoa(
                              *(struct in_addr *) & (currentHost->address->netmask)),
                    15);
...
void SLog(Packet *p, int scan_type, Portscanner *ps)
{
    ...
    { 
        ...
        snprintf(msg, STD_BUF, "%s TCP src: %s dst: %s sport: %u dport: %u "
    ...
    {
        snprintf(msg, STD_BUF, "%s UDP src: %s dst: %s sport: %u dport: %u "
                 "tgts: %u ports: %u event_id: %u\n", timestamp, src, dst, 
                 p->sp, p->dp, ps->target_count, ps->port_count, ps->event_id);
    }
    else if(p->icmph)
    {
        snprintf(msg, STD_BUF, "%s ICMP src: %s dst: %s type: %u code: %u "

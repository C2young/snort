/* $Id: parser.c,v 1.65 2002/08/26 21:10:07 roesch Exp $ */
/*
...
void ParseRulesFile(char *file, int inclevel)
{
    ...
    if(file_name == NULL)
    {
        FatalError("ERROR => ParseRulesFile strdup failed: %s\n", 
                strerror(errno));
    }
    ...
    if(stat(file_name, &file_stat) < 0) 
    {
        ...
        if(file_name == NULL)
        {
            FatalError("ERROR => ParseRulesFile calloc failed: %s\n", 
                       strerror(errno));
        }
    ...
    {
        FatalError("ERROR: Unable to open rules file: %s or %s\n", file, 
                   file_name);
    }
...
void ParseRule(FILE *rule_file, char *prule, int inclevel)
{
    ...
    if(!CheckRule(prule))
    {
        ErrorMessage("[!] ERROR: Unterminated rule in file %s, line %d\n", 
                file_name, file_line);
        ErrorMessage("   (Snort rules must be contained on a single line or\n");
        ErrorMessage("    on multuple lines with a '\\' continuation character\n");
        ErrorMessage("    at the end of the line,  make sure there are no\n");
        FatalError("    carriage returns before the end of this line)\n");
        return;
    }
    ...
    */
    ...
    {
	FatalError("ERROR %s (%d): Illegal direction specifier: %s", toks[4]);
    }
    ...
    switch(plugin->node_type)
    {
        case NT_OUTPUT_SPECIAL:
            ...
            if(!pv.log_cmd_override)
            {
                ...
                ErrorMessage("WARNING: command line overrides rules file login plugin!\n");
            plugin->func(pp_args);
            break;
        ...
        while(num_toks)
        {
            ...
            if(!strcasecmp(opts[0], "msg"))
            {
                if(one_msg == 0)
                {
                    ...
                    {
                        FatalError("\nERROR: %s(%d) => No argument passed to "
                ...
                {
#ifndef SOURCEFIRE /* TBD: Remove in 1.10 */
                    FatalError("ERROR: %s(%d) => Multiple %s arguments. "
...
struct VarEntry *VarDefine(char *name, char *value)
{
    ...
    if(value == NULL)
    {
        ErrorMessage("[!] ERROR %s (%d): Bad value in variable definition!\n",
                      file_name, file_line);
...
char *VarGet(char *name)
{
    ...
    FatalError("ERROR => Undefined variable name: (%s:%d): %s\n", 
...
void ParseConfig(char *rule)
{
    ...
    {
        if(!strcasecmp(args, "basic"))
        {
            pv.ghetto_msg_flag = GHETTO_BASIC;
        }
        else if(!strcasecmp(args, "url"))
        {
    ...
    else if(!strcasecmp(config, "min_ttl"))
    {
        ...
        {
            FatalError("Error: config min_ttl requires an argument\n", args);
        }
        return;
    }
    else if(!strcasecmp(config, "reference"))
    {
        ...
        {
            ErrorMessage("WARNING %s(%d) => Reference config without "
                    "arguments\n", file_name, file_line);
        }
        return;
    }

    ErrorMessage("Error: Unknown config directive: %s\n", rule);
...
void ParseDeclaredRuleType(char *rule)
{
    ...
    */
    if(strcmp("->", toks[4]) && strcmp("<>", toks[4]))
    {
	FatalError("ERROR %s (%d): Illegal direction specifier: %s\n",

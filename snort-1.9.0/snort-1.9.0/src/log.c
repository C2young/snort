/* $Id: log.c,v 1.103 2002/09/06 00:02:38 chrisgreen Exp $ */
/*
...
void PrintArpHeader(FILE * fp, Packet * p)
{
    ...
    switch(ntohs(p->ah->ea_hdr.ar_op))
    {
        case ARPOP_REQUEST:
            ...
            if(memcmp((char *) mac_src, (char *) p->ah->arp_sha, 6) 
                    != 0)
            {
...
void PrintICMPHeader(FILE * fp, Packet * p)
{
    ...
    switch(p->icmph->type)
    {
        ...
        case ICMP_DEST_UNREACH:
            ...
            switch(p->icmph->code)
            {
                ...
                case ICMP_PKT_FILTERED_NET:
                    fwrite("ADMINISTRATIVELY PROHIBITED NETWORK FILTERED", 45, 
                            1, fp);
                    break;

                case ICMP_PKT_FILTERED_HOST:
                    fwrite("ADMINISTRATIVELY PROHIBITED HOST FILTERED", 42, 
                            1, fp);
                    break;
        ...
        case ICMP_ROUTER_ADVERTISE:
            fwrite("ROUTER ADVERTISMENT", 19, 1, fp);
...
void PrintEapolKey(FILE * fp, Packet * p)
{
    ...
    fprintf(fp, " len: %d", ntohs(*((u_short *)&p->eapolk->length[0])));

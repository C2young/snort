...
/* $Id: spo_database.c,v 1.59 2002/09/17 19:18:50 danyliw Exp $ */
...
void Database(Packet *p, char *msg, void *arg, Event *event)
{
	...
	tmp2_len = 0;
        ...
        tmp1_len += snprintf(tmp1, MAX_QUERY_LENGTH-tmp1_len, "%s", "sig_name");
	tmp2_len += snprintf(tmp2, MAX_QUERY_LENGTH-tmp2_len, "'%s'", msg);
        if ( class_id > 0 )
        {
	    tmp1_len += snprintf(&tmp1[tmp1_len], MAX_QUERY_LENGTH-tmp1_len, "%s", ",sig_class_id");
	    tmp2_len += snprintf(&tmp2[tmp2_len], MAX_QUERY_LENGTH-tmp2_len, ",%u", class_id);
        } 

        if ( event->priority > 0 )
        {
            tmp1_len += snprintf(&tmp1[tmp1_len], MAX_QUERY_LENGTH-tmp1_len, "%s", ",sig_priority");
	    tmp2_len += snprintf(&tmp2[tmp2_len], MAX_QUERY_LENGTH-tmp2_len, ",%u", event->priority);
        }

        if ( event->sig_rev > 0 )
        {
            tmp1_len += snprintf(&tmp1[tmp1_len], MAX_QUERY_LENGTH-tmp1_len, "%s", ",sig_rev");
	    tmp2_len += snprintf(&tmp2[tmp2_len], MAX_QUERY_LENGTH-tmp2_len, ",%u", event->sig_rev);
        }

        if ( event->sig_id > 0 )
        {
            tmp1_len += snprintf(&tmp1[tmp1_len], MAX_QUERY_LENGTH-tmp1_len, "%s", ",sig_sid");
	    tmp2_len += snprintf(&tmp2[tmp2_len], MAX_QUERY_LENGTH-tmp2_len, ",%u", event->sig_id);
        }

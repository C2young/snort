...
/* $Id: spo_unified.c,v 1.32 2002/05/02 00:59:06 andrewbaker Exp $ */
...
void UnifiedInit(u_char *args)
{
    ...
    //LogMessage("UnifiedInit %s\n", args);
    if(unifiedConfig)
    {
        LogMessage("ERROR: unified can only be instantiated once\n");
        return;
    }
...
int UnifiedLogData(u_int32_t type, u_int32_t length, void *data)
{
    ...
    if(!unifiedConfig)
    {
        LogMessage("ERROR: Unified output not configured\n");
        return -1;
    }
